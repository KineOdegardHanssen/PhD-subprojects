...
infilename_base = 'chaingrid_quadratic_M9N101_ljdebye1.042_angle_Langevin_wall1.042_Kangle25_Kbond2000_T310_theta0is180_cutoffs3_sigma1_firstatomfixed' #'voxelmap_test_short'#'voxelmap_test_short_TESTII'#'spherepore_three'#'equal_distances_four'#'halving_distances'#'voxelmap_test_short'
#'''
...
'''
### More on names:
# /home/kine/Projects_PhD/P2_PolymerMD/Planar_brush/Fast_voxellation/chaingrid_quadratic_M9N101_ljdebye1.042_angle_Langevin_wall1.042_Kangle25_Kbond2000_T310_theta0is180_cutoffs3_sigma1_firstatomfixed/chaingrid_quadratic_M9N101_ljdebye1.042_angle_Langevin_wall1.042_Kangle25_Kbond2000_T310_theta0is180_cutoffs3_sigma1_firstatomfixed_step%i

foldername             = '/home/kine/Projects_PhD/P2_PolymerMD/Planar_brush/Fast_voxellation/'+infilename_base+'/'
plotfoldername         = foldername+'Plots/'
infilename_totalbase   = foldername
plotname_totalbase     = plotfoldername # Needed to shorten a bit
### For each frame:
...
## Read coordinates and stuff ### I should be able to recreate this, shouldn't I?

'''# Data format, meta file:
len_voxel: 2.19373
Nx: 55 Ny: 55 Nz: 46
maxx: 100.352 minx: -20.3034
maxy: 100.303 miny: -20.2771
maxz: 100 minz: 0
'''


metafilename = infilename_totalbase +'data.txt'
metafile     = open(metafilename, 'r')
metalines    = metafile.readlines()
Nx           = int(metalines[1].split()[1])
Ny           = int(metalines[1].split()[3])
Nz           = int(metalines[1].split()[5])


'''
infilename_x = infilename_totalbase +'_xind.txt' # Do I really need these?
infilename_y = infilename_totalbase +'_yind.txt'
infilename_z = infilename_totalbase +'_zind.txt'

infile_x = open(infilename_x, 'r')
infile_y = open(infilename_y, 'r')
infile_z = open(infilename_z, 'r')

xlines = infile_x.readlines()
ylines = infile_y.readlines()
zlines = infile_z.readlines()

words = xlines[0].split()

Ncols  = len(words) 
Nlines = len(xlines) # This will be the same for the indice matrices and the voxel value matrix 
...
for timeind in range(Nsteps):
    ...
    infilename_vmat = infilename_totalbase +'step'+timestep
    ...
    '''
    vmat = np.zeros((Nx,Ny,Nz))
    infile_vmat = open(infilename_vmat, 'r')
    vmat_lines = infile_vmat.readlines()
    
    
    matcounter = 0
    for i in range(Nx):
        for j in range(Ny): # New lines for every new combination of x and y.
            words = vmat_lines[matcounter].split()
            for k in range(Nz):
                vmat[i,j,k] = float(words[k]) # Is it really this easy?
            # Values of Nz will appear here
            #for Nz in range()
            matcounter += 1
    ...
    for thrind in range(Nthr):
        thr = thrs[thrind]
        thrindstring     = 'binary_thr' + str(thr) + '_timestep'+str(timestep)
        ...
        for i in range(Nr): 
            ...
            #radius += 2
            if plotconfig==True:
                plotname_3         = plotname_this + '_r_%i.png' % radius
                fig = plt.figure()
                ax = fig.add_subplot(111, projection='3d')
                pos = np.where(newimage==1)
                if issphere==True:
