    ...
    for i in range(Nx):
        for j in range(Ny):
            for k in range(Nz):
                ...
                outarray[n,3]  = voxval
                n+=1
                #print('Time to find closest atom:', time_after-time_before)
    return voxelvalues, voxmat, outarray
...
# After crashing:
#startframe  = 140
#indices     = np.arange(startframe,endframe+1)
...
# chaingrid_quadratic_M9N101_ljunits_gridspacing5_Langevin_Kangle14.0186574854529_Kbond140.186574854529_debye_kappa1_debyecutoff3_chargeelementary-1_effectivedielectric0.00881819074717447_T3_theta0is180
...
wallenergy  = 1.042
dielectric  = 0.00881819074717447#50 #1 2 10 100
...
#chaingrid_quadratic_M9N101_ljunits_gridspacing5_Langevin_Kangle14.0186574854529_Kbond140.186574854529_debye_kappa1_debyecutoff3_chargeelementary-1_effectivedielectric0.00881819074717447_T3_theta0is180
'''
infilename      = 'chaingrid_quadratic_M%iN%i_ljunits_gridspacing%i_Langevin_Kangle%.13f_Kbond%.12f_debye_kappa1_debyecutoff3_chargeelementary%i_effectivedielectric%.17f_T%i_theta0is180.lammpstrj' % (M,N,spacing,Kangle,Kbond,charge,dielectric,T)
outfilename_base = 'chaingrid_quadratic_M%iN%i_ljunits_gridspacing%i_Langevin_Kangle%.13f_Kbond%.12f_debye_kappa1_debyecutoff3_chargeelementary%i_effectivedielectric%.17f_T%i_theta0is180.lammpstrj' % (M,N,spacing,Kangle,Kbond,charge,dielectric,T)
#'''
...
for lindex in lineindices:
    ...
    if lindex==lineindices[0]: # Only make grid ONCE
        ...
        for i in range(M):
            zmax_temp = max(zs[i,:])
            #print('len(zs[',i,',:]):', len(zs[i,:])) # This part works.
            #print('zs[',i,',:]:',zs[i,:])
            #print('zmax_temp:',zmax_temp, '\n')
            zmaxes.append(zmax_temp)
        zmax_makegrid = np.mean(zmaxes)#max(zmaxes)
        ...
        '''# I have chosen to skip the buffer
    ...
    voxelvalues, voxmat, outarray = voxellate(voxN, Nx, Ny, Nz, x_centres, y_centres, z_centres, posvecs)
