...
#Nsecs = [] # Do I even need this? No.
Nidx  = [] # I think I will abandon using the actual segment names. Too much of a hassle and not that useful
Ls    = [] # Remember to make use of this!! 

morphfolder = 'figures/%i/' % testmodel # this is all I need.
morphfilename = folder+'%i_dendrites_section_segments.txt' % testmodel
morphfile = open(morphfilename,'r') # Reading in all the info
lines = morphfile.readlines()
for line in lines:
    words = line.split()
    Nidx.append(int(words[1]))
...
for k in range(len(Nidx)):
    for j in range(Nidx[k]): # Index Nidx. It should be an array now (or a list) # nah, need double loop
        ...
        max_value = max(V) # Can be dangerous if I have more than one peak. But I'll make sure I dont.
        peaktimes[j] = times[V.index(max_value)]
        
        # File and plot names
        if testmodel==496497595: # Is this test accurate anymore? I doubt it. CHECK!!!
            # Plot name:
