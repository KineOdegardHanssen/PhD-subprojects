...
def main(filename,idelay,idur,iamp):
    ...
    print('Finding the time constant...')
...
if __name__ == '__main__':
    ...
    outfilename_all = folder +'basps_%i_varycm_idur%i_iamp'%(testmodel,idur)+str(iamp)+'_Ra%i_vinit'%Ra+str(v_init)+'_Cm.txt' 
    plotname =  folder +'basps_%i_varycm_idur%i_iamp'%(testmodel,idur)+str(iamp)+'_Ra%i_vinit' %Ra+str(v_init)+'_Cm.png' 
    outname_all = open(outfilename_all,'w')
    for i in range(Ncm):
        cm = cms[i]
        print('cm:',cm)
        filename = folder+'basps_cm'+str(cm)+'_idur%i_iamp'%idur+str(iamp)+'_Ra%i_vinit' %Ra+str(v_init)+'_V.txt' 
        outfilename = folder +'basps_%i_cm'%testmodel+str(cm)+'_idur%i_iamp'%idur+str(iamp)+'_Ra%i_vinit' %Ra+str(v_init)+'_Cm.txt' 
