...
filename_cms = folder+'bas_varycm_idur%.1f_iamp'%idur+str(iamp)+'_Ra%i_vinit' %Ra+str(v_init)+'_propvel_vs_cm.txt'
figname_vel_vs_cm = folder+'bas_varycm_idur%.1f_iamp'%idur+str(iamp)+'_Ra%i_vinit' %Ra+str(v_init)+'_propvel_vs_cm.png'
file_cms = open(filename_cms,'w')
for i in range(Ncm):
    cm = cms[i]
    for j in range(1,Nidx):
        filename = folder+'bas_cm'+str(cm)+'_idur%.1f_iamp'%idur+str(iamp)+'_Ra%i_vinit' %Ra+str(v_init)+'_V_seg%i.txt' %j # For easy access
    ...
    figname = folder+'bas_cm'+str(cm)+'_idur'+str(idur)+'_iamp'+str(iamp)+'_Ra%i_vinit' %Ra+str(v_init)+'_V_dend_propagation.png'
    figname_vel = folder+'bas_cm'+str(cm)+'_idur'+str(idur)+'_iamp'+str(iamp)+'_Ra%i_vinit' %Ra+str(v_init)+'_V_dend_propvel.png'
    # Outfilename:
    outfilename_vel = folder+'bas_cm'+str(cm)+'_idur'+str(idur)+'_iamp'+str(iamp)+'_Ra%i_vinit' %Ra+str(v_init)+'_V_dend_propvel.txt'
