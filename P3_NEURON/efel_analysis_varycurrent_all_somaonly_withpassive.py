...
if __name__ == '__main__':
    ...
    idur      = 1000 # ms
    idelay    = 1
    iamps     = [0.0,0.02,0.04,0.06,0.08,0.1,0.12,0.14,0.16,0.18]
    v_init    = -70 #-86.5 # mV
    Ra        = 100 # -150
    somasize  = 15 
    ...
    # Set names
    outfolder = 'Results/IStim/Soma%i/Withpas/Vary_iamp/' % somasize
    outfilename_Nspikes = outfolder+'somaonly_soma%i_withpas_idur%i_cm'% (somasize,idur)+str(cm) +'_Nspikes_vs_iamp.txt'
    outfilename_APampl  = outfolder+'somaonly_soma%i_withpas_idur%i_cm'% (somasize,idur)+str(cm)+'_APampl_vs_iamp.txt'
    outfilename_APdhw   = outfolder+'somaonly_soma%i_withpas_idur%i_cm'% (somasize,idur)+str(cm)+'_APdurhalfwidth_vs_iamp.txt'
    plotname_Nspikes    = outfolder+'somaonly_soma%i_withpas_idur%i_cm'% (somasize,idur)+str(cm)+'_Nspikes_vs_iamp.png'
    plotname_APampl     = outfolder+'somaonly_soma%i_withpas_idur%i_cm'% (somasize,idur)+str(cm)+'_APampl_vs_iamp.png'
    plotname_APdhw      = outfolder+'somaonly_soma%i_withpas_idur%i_cm'% (somasize,idur)+str(cm)+'_APdurhalfwidth_vs_iamp.png'
    # make files
    ...
    for j in range(Namps):
        ...
        infolder = 'Results/IStim/Soma%i/Withpas/current_idur'%somasize+str(idur)+'_iamp'+str(iamps[j])+'/'
        filename = infolder+'somaonly_withpas_cm'+str(cm)+'_idur%i_iamp'%idur+str(iamps[j])+'_Ra'+str(Ra)+'_vinit'+str(v_init)+'_V.txt' 
