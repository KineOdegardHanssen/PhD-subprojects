...
model_folders = ['','CaHVA_Allen/SK_Allen/']
labels = ['base','(CaHVA, SK)*0.1']
Nmodels = len(model_folders)

outfolder = 'Compare/Soma%i/' % somasize
plotname = outfolder+'compare_properties_naf_kaf_CaHVA_Allen_SK_Allen0p1.png'
...
# Get names
infolder = model_folders[0]+'Results/Soma%i/' % somasize
...
ax1.plot(iamp_Nspikes,Nspikes,label=labels[0])
ax2.errorbar(iamp_AP_ampl,avg_AP_ampl,yerr=rms_AP_ampl,label=labels[0])
ax3.errorbar(iamp_AP_mins,avg_AP_mins,yerr=rms_AP_mins,label=labels[0])
ax4.errorbar(iamp_AP_halfwidth,avg_AP_halfwidth,yerr=rms_AP_halfwidth,label=labels[0])
...
gnafs   = 1.0
gkafs   = 1.0
gsks    = [0.1,0.2,1.0]#[0.1,0.2,0.5,1.0]
gcahvas = [0.1,0.2,1.0,2.0]#[0.1,0.2,0.5,1.0]
gpases  = 1.0
...
figname =   'Compare/Soma%i/' % somasize+'base_and_'+outnamestring+'.png'
...
for gcahva in gcahvas: # Ugh, this was not that general after all...
    for gsk in gsks:
        ...
        # Set names
        infolder = model_folders[1]+'Results/Soma%i/' % somasize
