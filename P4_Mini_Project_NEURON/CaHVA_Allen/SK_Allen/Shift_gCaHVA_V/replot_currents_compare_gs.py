...
if __name__ == '__main__':
    ...
    gkaf    = 1.0
    gcahva  = 0.2
    gsks    = [0.1,0.5] # Available: [0.1,0.5,1.0,1.5,2.0]
    gpas    = 1.0
    ...
    for gsk in gsks:
        ...
        if plotnaf==True:
            plt.plot(time,I_naf_this,color='tab:blue',label=r'$I_{\mathregular{naf}}$, %s$\bar{g}_{\mathregular{SK}}$' % str(gsk),linewidth=thiswidth,linestyle=thislinestyle)
        if plotkaf==True:
            plt.plot(time,I_kaf_this,color='tab:orange',label=r'$I_{\mathregular{kaf}}$, %s$\bar{g}_{\mathregular{SK}}$' % str(gsk),linewidth=thiswidth,linestyle=thislinestyle)
        if plotsk==True:
            plt.plot(time,I_SK_this,color='tab:gray',label=r'$I_{\mathregular{SK}}$, %s$\bar{g}_{\mathregular{SK}}$' % str(gsk),linewidth=thiswidth,linestyle=thislinestyle)
        if plotcahva==True:
            plt.plot(time,I_CaHVA_this,color='tab:red',label=r'$I_{\mathregular{CaHVA}}$, %s$\bar{g}_{\mathregular{SK}}$' % str(gsk),linewidth=thiswidth,linestyle=thislinestyle)
        if plotcap==True:
            plt.plot(time,I_cap_this,color='tab:green',label=r'$I_{\mathregular{cap}}$, %s$\bar{g}_{\mathregular{SK}}$' % str(gsk),linewidth=thiswidth,linestyle=thislinestyle)
        if plotmem==True:
            plt.plot(time,I_mem_this,color='tab:olive',label=r'$I_{\mathregular{mem}}$, %s$\bar{g}_{\mathregular{SK}}$' % str(gsk),linewidth=thiswidth,linestyle=thislinestyle)
        if plotpas==True:
            plt.plot(time,I_pas_this,color='tab:purple',label=r'$I_{\mathregular{pas}}$, %s$\bar{g}_{\mathregular{SK}}$' % str(gsk),linewidth=thiswidth,linestyle=thislinestyle)
        i+=1
    plt.xlabel('Time [ms]')
