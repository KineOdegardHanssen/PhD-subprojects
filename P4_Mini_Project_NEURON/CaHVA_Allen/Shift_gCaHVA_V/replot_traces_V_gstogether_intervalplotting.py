...
############ SET SOME INTERVAL TO MAKE IT EASIER TO SEE ###############
startind = 13000
endind   = 25000
## These were good for one peak + some more:
#startind = 12500
#endind   = 15000
#######################################################################
...
cm       = 1.0
iamp     = 0.005
gcahvas  = [0.1,1.0,1.5]#[0.1,0.5,1.0,1.5]
...
plotname = outfolder + 'selectedvshift_selectedcahva_iamp'+str(iamp)+'_subplotV_gt_traces_Ng%i_interval%ito%i.png' % (Ng,startind,endind)
...
ax1.set_title(r'$V_{\mathregular{Shift}}$ = %i mV' % vshifts[0],fontsize=18)
ax2.set_title(r'$V_{\mathregular{Shift}}$ = %i mV' % vshifts[1],fontsize=18)
ax3.set_title(r'$V_{\mathregular{Shift}}$ = %i mV' % vshifts[2],fontsize=18)
ax4.set_title(r'$V_{\mathregular{Shift}}$ = %i mV' % vshifts[3],fontsize=18)
...
for i in range(Ng):
    ...
    ax1.plot(time[startind:endind], voltage[startind:endind], label=r'%s$\bar{g}_{\mathregular{CaHVA}}$' % str(gcahvas[i]),linewidth=2,color=colors[i])
...
for i in range(Ng):
    time    = times_this[i]
    voltage = voltages_this[i]
    ax2.plot(time[startind:endind], voltage[startind:endind], label=r'%s$\bar{g}_{\mathregular{CaHVA}}$' % str(gcahvas[i]),linewidth=2,color=colors[i])
...
for i in range(Ng):
    time    = times_this[i]
    voltage = voltages_this[i]
    ax3.plot(time[startind:endind], voltage[startind:endind], label=r'%s$\bar{g}_{\mathregular{CaHVA}}$' % str(gcahvas[i]),linewidth=2,color=colors[i])
...
for i in range(Ng):
    time    = times_this[i]
    voltage = voltages_this[i]
    ax4.plot(time[startind:endind], voltage[startind:endind], label=r'%s$\bar{g}_{\mathregular{CaHVA}}$' % str(gcahvas[i]),linewidth=2,color=colors[i])
