...
plotname = outfolder + 'selectedvshift_selectedcahva_iamp'+str(iamp)+'_subplotg_Vt_traces_Ng%i.png' % Ng
...
for i in range(Nv):
    ...
    ax1.plot(time, voltage, label=r'$\bar{V}_{\mathregular{Shift}}$=%s mV' % str(vshifts[i]),linewidth=2,color=colors[i])
...
for i in range(Nv):
    time    = times_this[i]
    voltage = voltages_this[i]
    ax2.plot(time, voltage, label=r'$\bar{V}_{\mathregular{Shift}}$=%s mV' % str(vshifts[i]),linewidth=2,color=colors[i])
...
for i in range(Nv):
    time    = times_this[i]
    voltage = voltages_this[i]
    ax3.plot(time, voltage, label=r'$\bar{V}_{\mathregular{Shift}}$=%s mV' % str(vshifts[i]),linewidth=2,color=colors[i])
...
for i in range(Nv):
    time    = times_this[i]
    voltage = voltages_this[i]
    ax4.plot(time, voltage, label=r'$\bar{V}_{\mathregular{Shift}}$=%s mV' % str(vshifts[i]),linewidth=2,color=colors[i])
