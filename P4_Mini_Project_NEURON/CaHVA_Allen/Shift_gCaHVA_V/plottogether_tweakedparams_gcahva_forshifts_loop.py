...
for vshift in vshifts:
    ...
    outfolder = 'Compare/'
    figname   = outfolder+outnamestring+'_Vshift'+str(vshift)+'_properties.png'
    ...
    for gcahva in gcahvas:
        ...
        if vary_Ca_HVA==True:
            ...
            thelabel   = thelabel + r'%.2f$\bar{g}_\mathregular{CaHVA}$, ' % gcahva
        if vary_gpas==True: 
        ...
        infilename_APampl  = folder+'somaHjorth_idur%i_varyiamp'% (idur)+'_manual_cm'+str(cm)+namestring+'_Vmax_vs_I_s'+str(skiptime)+'.txt'
        infilename_APmins  = folder+'somaHjorth_idur%i_varyiamp'% (idur)+'_manual_cm'+str(cm)+namestring+'_Vmin_vs_I_s'+str(skiptime)+'.txt'
        ...
        if numpy.sum(Nspikes)!=0:
            ...
            ax2.errorbar(I_maxima,maxima, yerr=maxima_rms, capsize=2, label=thelabel,linewidth=thislinewidth,color=colors[i])
            ax3.errorbar(I_minima,minima, yerr=minima_rms, capsize=2, label=thelabel,linewidth=thislinewidth,color=colors[i])
            ax4.errorbar(I_dur,dur, yerr=dur_rms, capsize=2, label=thelabel,linewidth=thislinewidth,color=colors[i])
        i+=1
