...
cm       = 1.0
iamp     = 0.005
gcahvas  = [0.1,0.5,1.0,1.5]#[0.1,0.5,1.0,1.5]
vshifts  = [-5,0,5]#[-50,-45,-40,-35,-30,-25,-20,-15,-10,-5,0,5,10,15,20,25,30,35,40,45,50]
Ng       = len(gcahvas)
Nv       = len(vshifts)
...
plotname = outfolder + 'selectedvshift_selectedcahva_iamp'+str(iamp)+'_subplotg_Vt_traces_Ng%i_interval%ito%i.png' % (Ng,startind,endind)
...
for i in range(Ng):
    ...
    for j in range(Nv):
        ...
        print('Step ', j+1, ' of', Ng, 'cm:', cm, 'vshift:', vshift)    
...
for i in range(Nv):
    ...
    ax1.plot(time[startind:endind], voltage[startind:endind], label=r'$\bar{V}_{\mathregular{Shift}}$=%s mV' % str(vshifts[i]),linewidth=2,color=colors[i])
...
for i in range(Nv):
    time    = times_this[i]
    voltage = voltages_this[i]
    ax2.plot(time[startind:endind], voltage[startind:endind], label=r'$\bar{V}_{\mathregular{Shift}}$=%s mV' % str(vshifts[i]),linewidth=2,color=colors[i])
...
for i in range(Nv):
    time    = times_this[i]
    voltage = voltages_this[i]
    ax3.plot(time[startind:endind], voltage[startind:endind], label=r'$\bar{V}_{\mathregular{Shift}}$=%s mV' % str(vshifts[i]),linewidth=2,color=colors[i])
...
for i in range(Nv):
    time    = times_this[i]
    voltage = voltages_this[i]
    ax4.plot(time[startind:endind], voltage[startind:endind], label=r'$\bar{V}_{\mathregular{Shift}}$=%s mV' % str(vshifts[i]),linewidth=2,color=colors[i])
