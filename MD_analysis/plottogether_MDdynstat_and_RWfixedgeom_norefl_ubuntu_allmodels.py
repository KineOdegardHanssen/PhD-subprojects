...
def notmonosph(phi):
    return 1./np.sqrt(phi) # phi**(-1/2.)

def pshimd_spherepacking(phi):
    return phi**(-1/3.)

def overlapping_spheres(phi):
    return 1-np.log(phi)/2.

def overlapping_cylinders(phi):
    return 1-np.log(phi)

def het_cat(phi):
    return phi/(1-(1-phi)**(1/3.))

def catex_resin_membr(phi):
    return ((2.-phi)/phi)**2
...
rsphere  = 1.0 #0.8
sigma    = 0.5 # Change this for the modelling part
...
if big==False:
    ### d
    plotname_d     = endlocation_static+'D_vs_d_dyn_vs_stat_vs_RWgeom_norefl_rsphere'+str(rsphere)+'_allmodels_modelr'+str(sigma)+'.png'
    ...
    ### d 
    plotname_d     = endlocation_static+'D_vs_d_dyn_vs_stat_vs_RWgeom_big_norefl_rsphere'+str(rsphere)+'_allmodels_modelr'+str(sigma)+'.png'
    plotname_cut_d = endlocation_static+'D_vs_d_dyn_vs_stat_cut_vs_RWgeom_big_norefl_rsphere'+str(rsphere)+'_allmodels_modelr'+str(sigma)+'.png' ## Use this?
...
# Models: # Should I invert them first? (doesn't really matter)
# Ordered packing
Ds_op_rw      = 1./ordered_packings(porosity_rw) # Diffusion constant for ordered packings
Ds_op_stat    = 1./ordered_packings(porosity_stat)
Ds_op_dyn     = 1./ordered_packings(porosity_dyn)
Ds_op_f       = 1./ordered_packings(porosity_f)
# Hyperbola of revolution
Ds_hr_rw      = 1./hyp_rev(porosity_rw)
Ds_hr_stat    = 1./hyp_rev(porosity_stat)
Ds_hr_dyn     = 1./hyp_rev(porosity_dyn)
Ds_hr_f       = 1./hyp_rev(porosity_f)
# Not monosized spheres
Ds_nm_rw      = 1./notmonosph(porosity_rw_positive)
Ds_nm_stat    = 1./notmonosph(porosity_stat_positive)
Ds_nm_dyn     = 1./notmonosph(porosity_dyn_positive)
Ds_nm_f       = 1./notmonosph(porosity_f_positive)
# pshimd_spherepacking
Ds_ps_rw      = 1./pshimd_spherepacking(porosity_rw_positive)
Ds_ps_stat    = 1./pshimd_spherepacking(porosity_stat_positive)
Ds_ps_dyn     = 1./pshimd_spherepacking(porosity_dyn_positive)
Ds_ps_f       = 1./pshimd_spherepacking(porosity_f_positive)
# Overlapping spheres
Ds_os_rw      = 1./overlapping_spheres(porosity_rw_positive)
Ds_os_stat    = 1./overlapping_spheres(porosity_stat_positive)
Ds_os_dyn     = 1./overlapping_spheres(porosity_dyn_positive)
Ds_os_f       = 1./overlapping_spheres(porosity_f_positive)
# Overlapping cylinders
Ds_oc_rw      = 1./overlapping_cylinders(porosity_rw_positive)
Ds_oc_stat    = 1./overlapping_cylinders(porosity_stat_positive)
Ds_oc_dyn     = 1./overlapping_cylinders(porosity_dyn_positive)
Ds_oc_f       = 1./overlapping_cylinders(porosity_f_positive)
# Heterogeneous catalyst
Ds_hc_rw      = 1./het_cat(porosity_rw) # Diffusion constant for ordered packings
Ds_hc_stat    = 1./het_cat(porosity_stat)
Ds_hc_dyn     = 1./het_cat(porosity_dyn)
Ds_hc_f       = 1./het_cat(porosity_f)
# Cation exchange resin memrane
Ds_rm_rw      = 1./catex_resin_membr(porosity_rw) # Diffusion constant for ordered packings
Ds_rm_stat    = 1./catex_resin_membr(porosity_stat)
Ds_rm_dyn     = 1./catex_resin_membr(porosity_dyn)
Ds_rm_f       = 1./catex_resin_membr(porosity_f)
...
ax.plot(spacings_dyn,Ds_op_dyn, '--', label='Ordered packings') # I have most values for the dynamic brush
ax.plot(spacings_dyn,Ds_hr_dyn, '--', label='Hyperbola of revolution') # 2
ax.plot(d_dyn_positive,Ds_nm_dyn, '--', label='Not monosized spheres') 
ax.plot(d_dyn_positive,Ds_ps_dyn, '--', label='Monodisperse sphere packing') ###§ 
ax.plot(d_dyn_positive,Ds_os_dyn, '--', label='Overlapping spheres') # 5
ax.plot(d_dyn_positive,Ds_oc_dyn, '--', label='Overlapping cylinders') 
ax.plot(spacings_dyn,Ds_hc_dyn, '--', label='Heterogeneous catalyst') 
ax.plot(spacings_dyn,Ds_rm_dyn, '--', label='Cation-exchange resin memrane') 
...
D_rmsd_nm_rw   = rmsd(D_rw_positive,Ds_nm_rw)
D_rmsd_nm_stat = rmsd(D_stat_positive,Ds_nm_stat)
D_rmsd_nm_dyn  = rmsd(D_dyn_positive,Ds_nm_dyn)
D_rmsd_nm_f    = rmsd(D_f_positive,Ds_nm_f)
# pshimd_spherepacking 4
D_rmsd_ps_rw   = rmsd(D_rw_positive,Ds_ps_rw)
D_rmsd_ps_stat = rmsd(D_stat_positive,Ds_ps_stat)
D_rmsd_ps_dyn  = rmsd(D_dyn_positive,Ds_ps_dyn)
D_rmsd_ps_f    = rmsd(D_f_positive,Ds_ps_f)
# Overlapping spheres 5
D_rmsd_os_rw   = rmsd(D_rw_positive,Ds_os_rw)
D_rmsd_os_stat = rmsd(D_stat_positive,Ds_os_stat)
D_rmsd_os_dyn  = rmsd(D_dyn_positive,Ds_os_dyn)
D_rmsd_os_f    = rmsd(D_f_positive,Ds_os_f)
# Overlapping cylinders 6
D_rmsd_oc_rw   = rmsd(D_rw_positive,Ds_oc_rw)
D_rmsd_oc_stat = rmsd(D_stat_positive,Ds_oc_stat)
D_rmsd_oc_dyn  = rmsd(D_dyn_positive,Ds_oc_dyn)
D_rmsd_oc_f    = rmsd(D_f_positive,Ds_oc_f)
