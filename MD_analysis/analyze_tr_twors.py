...
if long==False:
    plotname_dots_lines  = endlocation+ 'trs_' + hstring + filestext+'_dotline_dynamic_correct.png'
    plotname_loglog      = endlocation+ 'trs_' + hstring + filestext+'_loglog_dynamic_correct.png'
    plotname_ylog        = endlocation+ 'trs_' + hstring + filestext+'_ylog_dynamic_correct.png'
else:
    plotname_dots_lines  = endlocation+ 'trs_' + hstring + filestext+'_dotline_long_dynamic_correct.png'
    plotname_loglog  = endlocation+'trs_'+ hstring + filestext +'_loglog_long_dynamic_correct.png'
    plotname_ylog        = endlocation+'trs_'+ hstring + filestext+'_ylog_long_dynamic_correct.png'
...
figure()
for i in range(Nh):
    plt.plot(edges[i],hists[i], '-o', label='r=%i' % tesths[i])
plt.xlabel(r'$t_r$ [s]')
plt.ylabel(r'No. of arrivals/$N_{sims}$') 
plt.title('$t_r$ for d = %i nm' % spacing) 
plt.legend(loc='upper right')
plt.savefig(plotname_dots_lines)

figure()
for i in range(Nh):
    plt.loglog(edges[i],hists[i], '-o', label='r=%i' % tesths[i])
plt.xlabel(r'$t_r$ [s]')
plt.ylabel(r'No. of arrivals/$N_{sims}$') 
plt.title('$t_r$ for d = %i nm' % spacing) 
plt.legend(loc='upper right')
