...
if spacesims==True:
    outfilename  = 'persistence_length_neighbourbonds_Kangle%i_Kbond%i_T%i_debye_kappa1_debyecutoff3_N%i_varyspacing.txt' % (kangle,kbond,T,N)
    outfile      = open(outfilename, 'w')
...
for i in range(Narray):
    ...
    for line in lines:
        ...
        if len(words)!=0:
            counter      += 1
            pl_av  +=float(words[0])
            pl_rms +=float(words[1])
            pl_this[counter-1] = float(words[0])
    ...
    pl_rms2 = 0
    for j in range(M):
        pl_rms2 += (pl_this[j]-pl_av)**2
    pl_rms2 = np.sqrt(pl_rms2/(M*(M-1)))
