...
outfilename = 'testsystem_chaingrid_rotatingarc_withtime.lammpstrj'
...
for ind1 in range(100):
    ...
    for j in range(M):
        ...
        for i in range(N):
            x0 = 0
            y0 = np.cos(np.pi/((N-1)*2.0)*i+0.1*ind1) # Should make a set angle between each bond
            z0 = np.sin(np.pi/((N-1)*2.0)*i+0.1*ind1) # But the angle should be small enough that the chain does not make a ring (i.e. no overlap of bonds)
            ...
            k += 1
            l += 1
    for ind2 in range(M*(N+1)):
        # ITEM: ATOMS id type mol x y z vx vy vz
        outfile.write('%i %i %i %.16e %.16e %.16e %.16e %.16e %.16e\n' % ((ind2+1), atomtypes_all[ind2], molID[ind2], xpos[ind2], ypos[ind2], zpos[ind2], vxs[ind2], vys[ind2], vzs[ind2]))
