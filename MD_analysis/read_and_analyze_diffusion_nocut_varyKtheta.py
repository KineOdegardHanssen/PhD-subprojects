...
for Ktheta in Kthetas:
    ...
    endlocation   = 'C:/Users/Kine/Documents/Projects_PhD/P2_PolymerMD/Planar_brush/Diffusion_bead_near_grid/Spacing'+str(spacing)+'/damp%i_diffseedLgv/' % damp +parentfolder+ 'Sigma_bead_' +str(psigma) + '/VaryKtheta/Ktheta'+str(Ktheta)+'/Nocut/'
    if cutit==True:
    ...
    plt.title('RMSD in bulk, Ktheta = %i, SI' % Ktheta)
    plt.tight_layout()
    ...
    plt.title('$dz^2$ in bulk, Ktheta = %i, SI' % Ktheta)
    plt.tight_layout()
    plt.legend(loc='upper left')
    ...
    plt.title('$dx^2+dy^2$ in bulk, Ktheta = %i, SI' % Ktheta)
    plt.tight_layout()
    plt.legend(loc='upper left')
