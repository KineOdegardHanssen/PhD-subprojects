...
startlines = 25 # The number of lines we are going to read from the data.-infile and write to the data.-outfile # I will update this with the number of atoms later, so it should be sort of automatic
...
outfilename = 'data.bead_wsubstr_eq_N909_d'+str(spacing)+'_charge%i_mass1_Ncr%i_pad%i_nocrosslinkers_file' % (charge,Ncr,pad)
...
for i in range(Nfiles):
    ...
    for j in range(startlines-1,startlines+N_atoms-1):
        ...
        if len(words)>0:
            ...
            #print('I broke.')
            break
    outfile.write('%i %i 3 %i %.16e %.16e %.16e\n' % (freebead_number, max(molID)+1,charge,xran[i],yran[i],zran[i])) # atom-ID molecule-ID atom-type q x y z
